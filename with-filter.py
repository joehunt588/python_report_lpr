...
print('number of chunk:', num_chunks)
...
def generate_pdf_chunk(chunk_idx):
    ...
    # print(df.iloc[:,3])
    # json_data = df.iloc[:,3]
    # def extract_license_plate(json_string):
    #     import json
    #     data = json.loads(json_string)
    #     return data['metadata']['license_plate']
    #
    # # Apply the function to each element in the Series
    # license_plates = json_data.apply(extract_license_plate)
    #
    # # Display the extracted license plates
    # print(license_plates)

    # Whitelist
    # whitelist = ['BMY4014', 'VDN9192']

    # Filter the DataFrame based on the whitelist
    # filtered_df = df[df.iloc[:,3].isin(whitelist)]
    # print(filtered_df)
    # filtered_df = df[df['ColumnName'].isin(whitelist)]
