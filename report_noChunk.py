...
# pandadf = pd.concat(map(pd.read_csv, [file_path]))
# df = pandadf[pandadf['filename'] == file_path]
df = pd.read_csv(file_path)
df_white = pd.read_csv(file_path_whitelist)

# snippet pandas filter by date
...
print('Done query in pandas')
# Set the number of rows  PDF chunk
rows_per_chunk = 50
num_threads = 4
total_rows = len(df)
